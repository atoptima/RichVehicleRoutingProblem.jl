...
function generate_data_random_tsp(n::Int)
    ...
    for i in 1:length(pickup_points)
        p = pickup_points[i]
        req = ShipmentRequest(string("client_", p.index), i, 1,
              "unique_specific_product",
              false, 0.0, 0.0, 0.0, false, 0, [string("PickupPoint_", p.index)],
...
function generate_full_data_random(n::Int)
    ...
    for i in 1:length(delivery_points)
        ...
        req = ShipmentRequest(
            ...
            i + length(pickup_points), 2,
            "unique_specific_product",
            false, 0.0, 0.0, 0.0, false, 0, String[],
            [string("DeliveryPoint_", d.index)],
