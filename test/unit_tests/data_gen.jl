...
function generate_data_random_tsp_tests()
    ...
    for i in 1:length(data.requests)
        ...
        @test req.pickup_service_time == 0.0
        @test req.delivery_service_time == 0.0
...
function generate_random_depot_tests()
    ...
    @test typeof(depot) == RVRP.DepotPoint
...
function generate_random_vehicle_sets_tests()
    ...
    depot_points = [RVRP.generate_random_depot(i+1, i) for i in 1:2]
    vs = RVRP.generate_random_vehicle_sets(3, v_cats, depot_points)
    @test length(vs) == 3
