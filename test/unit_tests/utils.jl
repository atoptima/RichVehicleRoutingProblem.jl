...
function gather_all_locations_tests()
    ...
    p = RVRP.Pickup("", 1, l2, "", 0.0, 0.0, RVRP.TimeWindow[], 0.0)
    d = RVRP.Delivery("", 1, l1, "", 0.0, 0.0, RVRP.TimeWindow[], 0.0)
    ship = RVRP.Shipment("", 1, 0.0, p, d, 0.0)
    push!(data.pickups, p)
...
function set_indices_tests()
    ...
    data = RVRP.generate_data_random_tsp(10)
    p = RVRP.Pickup("", -1, l2, "", 0.0, 0.0, RVRP.TimeWindow[], 0.0)
    d = RVRP.Delivery("", 121, l1, "", 0.0, 0.0, RVRP.TimeWindow[], 0.0)
    ship = RVRP.Shipment("", -13, 0.0, p, d, 0.0)
    push!(data.pickups, p)
    push!(data.deliveries, d)
    push!(data.shipments, ship)
